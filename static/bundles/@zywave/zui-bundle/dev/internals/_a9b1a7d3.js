...
const style = css`:host{all:unset;contain:layout;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,Arial,sans-serif,"Apple Color Emoji","Segoe UI Emoji","Segoe UI Symbol";font-size:var(--zui-font-size, 0.875rem);font-weight:400;line-height:var(--zui-line-height, 1.6);box-sizing:border-box;-webkit-text-size-adjust:100%}:host([hidden]){display:none}*,*::before,*::after{box-sizing:border-box}`;

const trackers = new WeakMap();
function onResetUtilizationTracker() {
  instance.track('instance', this.tagName.toLowerCase());
}
class ZuiBaseElement extends LitElement {
  static {
    this.styles = [style];
  }
  connectedCallback() {
    super.connectedCallback();
    instance.track('instance', this.tagName.toLowerCase());
    const tracker = onResetUtilizationTracker.bind(this);
    trackers.set(this, tracker);
    instance.addEventListener('reset', tracker);
  }
  disconnectedCallback() {
    super.disconnectedCallback();
    instance.untrack('instance', this.tagName.toLowerCase());
    const tracker = trackers.get(this);
    if (tracker) {
      instance.removeEventListener('reset', tracker);
...
/**
 * Utility function to update the custom state of an element. Necessary as Chrome upgrades to the new standard and the old syntax is deprecated
 * @param elInternals
 * @param action
 * @param state
 */
function updateCustomState(elInternals, action, state) {
  try {
    // @ts-ignore
...
const SUPPORTS_FACE = !!window.ElementInternals?.prototype?.setFormValue;
if (!SUPPORTS_FACE) {
  /* eslint-disable-next-line no-console */
  console.warn("Form association isn't supported in this browser. ZUI controls won't participate in native form submissions.");
}
 ...
 */
class ZuiFormAssociatedElement extends ZuiBaseElement {
  static {
    this.shadowRootOptions = {
      ...ZuiBaseElement.shadowRootOptions,
      delegatesFocus: true
    };
  }
  /**
  ...
  get form() {
    // @ts-ignore attachInternals not supported yet
    if (this.#internals) {
      // @ts-ignore attachInternals not supported yet
  ...
  /**
   * The name of this element that is associated with form submission
   */
  get name() {
  ...
  /**
   * Returns the error message that would be displayed if the element was to be checked for validity
   */
  get validationMessage() {
    return this.#internals?.validationMessage ?? '';
  }
  /**
   * This is required when a consumer utilizes {@link setCustomValidity}
   */
  #priorValidationMessage;
  #focusControl;
  #internals;
  get _formSubmitButton() {
    if (this.form) {
      return this.form.querySelector('button[type="submit"],button:not([type]),input[type="submit"]');
    } else {
      return null;
    }
  }
  constructor() {
    super();
    /**
     * Represents whether a user can make changes to this element; if true, the value of this element will be excluded from the form submission
     */
    this.disabled = false;
    /**
     * Represents whether a user can make changes to this element; the value of this element will still be included in the form submission
     */
    this.readOnly = false;
    /**
     * If true, this element will be focused when connected to the document
     */
  ...
  firstUpdated(changedProps) {
    ...
    // this is necessary to workaround the lack of :user-invalid support in ElementInternals
    // see https://github.com/whatwg/html/issues/9639
    this.addEventListener('blur', () => {
      updateCustomState(this.#internals, 'add', 'tmp-user-interacted');
    });
    if (this.autofocus) {
      this.focus();
    }
  ...
  formResetCallback() {
    throw new Error('You must set a formResetCallback() for each ZUI form custom element.');
  }
  checkValidity() {
   ...
   * @param message - The error message to be displayed when the element is checked for validity
   */
  setCustomValidity(message) {
    if (message) {
      ...
      this.#internals?.setValidity({
        ...cloneValidityState(this.validity),
        customError: true
      }, message, this.#focusControl);
    } else if (this.#priorValidationMessage) {
      this.#internals?.setValidity({
        ...cloneValidityState(this.validity),
        customError: false
      }, this.#priorValidationMessage, this.#focusControl);
    } else {
      this.#internals?.setValidity({});
    }
  }
  ...
  _setValidity(validity, message) {
    message ? this.#internals?.setValidity?.(validity, message, this.#focusControl) : this.#internals?.setValidity?.({});
  }
  _mapValidationMessage(message, validityState, validationMessageMap) {
    for (const entry of Object.entries(validityState)) {
      if (!entry[1]) {
        continue;
      }
      const key = entry[0];
      const messageKey = validationMessageMap[key];
      if (messageKey) {
        const validationAttribute = `validation-message-${messageKey}`;
        if (this.hasAttribute(validationAttribute)) {
          return this.getAttribute(validationAttribute);
        }
      }
    }
    return message;
...
__decorate([property({
  type: Boolean,
  reflect: true
})], ZuiFormAssociatedElement.prototype, "disabled", void 0);
__decorate([property({
  attribute: 'readonly',
  type: Boolean,
  reflect: true
})], ZuiFormAssociatedElement.prototype, "readOnly", void 0);
__decorate([property({
  type: Boolean,
  reflect: true
})], ZuiFormAssociatedElement.prototype, "autofocus", void 0);
...
export { Directive as D, PartType as P, ZuiBaseElement as Z, _$LH as _, ZuiFormAssociatedElement as a, classMap as b, css as c, noChange as d, directive as e, queryAssignedElements as f, desc as g, html as h, instance as i, nothing as n, property as p, query as q, render as r, svg as s, updateCustomState as u };
